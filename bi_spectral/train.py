utimefrom __future__ import print_function, division
import sys
import os
import datetime
import numpy as np
...
from tensorflow.keras.layers import Input, Concatenate, BatchNormalization, Activation, Conv1D, MaxPool1D, UpSampling1D
...
class temporal_network():
    def __init__(self,
        ...
        self.utime = self.multiclass_unet1D()
        self.utime.compile(loss=weighted_categorical_crossentropy(self.alpha),
    ...
    def train(self,epochs, batch_size):
        ...
        for iter in range(epochs*Niter_epoch):
            ...
            g_loss = self.utime.train_on_batch(X, y)
    ...
    def chek_training(self,iter,gloss=0):
        ...
        Ypredict = self.utime.predict(Xval)
        ...
        self.utime.save('%s/models/model_%.8d_loss_%.5f_val_%.5f.h5'%(self.filepath_save, iter,gloss,eval_valid_loss))
...
if __name__ == '__main__':
    ...
    if os.path.exists(pretrain):
        ...
        net.utime=load_model(pretrain,custom_objects=DEF_CUSTOM_OBJECTS)
        print('Done')
        net.utime.compile(loss=weighted_categorical_crossentropy(alpha),
                               optimizer=tf.keras.optimizers.SGD(lr=initial_lr, momentum=0.9),
                               metrics=[weighted_categorical_crossentropy(alpha)])
        print('---------------------------------')
    else:
        print('---------------------------------')
    ...
    stringlist = []
    net.utime.summary(print_fn=lambda x: stringlist.append(x))
    short_model_summary = "\n".join(stringlist)
